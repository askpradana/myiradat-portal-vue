...
const deleteFunc = () => {
  if (BrowserName.value === 'Delete All') {
    mutate()
  } else {
...
<template>
  <AlertDialogRoot v-model:open="open">
    <AlertDialogTrigger>
      <Button
        title="Delete all session except current session"
        variant="destructive"
        size="sm"
        class="hover:bg-red-500"
      >
        <Trash2 :size="12" /> Delete All Session
      </Button>
    </AlertDialogTrigger>
    <AlertDialogPortal>
      <AlertDialogOverlay
      ...
      >
        ...
        <AlertDialogDescription class="mb-5 text-sm leading-normal">
          Are you sure you want to delete all session
          <span class="font-semibold">except current session</span>? <br />
          Please enter the word according to this term: <br />
          <div class="w-full text-center my-4">
            <span class="text-red-600 font-semibold text-xl">Delete All</span>
          </div>
