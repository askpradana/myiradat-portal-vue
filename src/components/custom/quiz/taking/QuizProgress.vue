...
<template>
  <div class="space-y-4">
    ...
    <div v-if="!mobileMode && showSteps && total <= 20" class="hidden md:block">
      <div class="flex items-center gap-1 flex-wrap">
        <div v-for="step in steps" :key="step.number" class="flex items-center">
            ...
            :class="[
              'flex items-center justify-center w-8 h-8 rounded-full text-xs font-medium transition-colors',
              {
                ...
                'bg-green-100 text-green-700 border border-green-300':
                  step.isAnswered && !step.isCurrent,
                'bg-muted text-muted-foreground border border-border':
                  !step.isAnswered && !step.isCurrent,
              },
            ]"
          >
            <CheckCircle2 v-if="step.isAnswered && !step.isCurrent" class="h-4 w-4" />
            <Circle v-else-if="!step.isAnswered && !step.isCurrent" class="h-4 w-4" />
          ...
          <div
            ...
            :class="['w-2 h-0.5 mx-1', step.isAnswered ? 'bg-green-300' : 'bg-border']"
          />
        </div>
      </div>
    </div>
    ...
    <div v-else-if="!mobileMode && showSteps && total > 20" class="hidden md:block">
